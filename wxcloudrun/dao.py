...
def search_friends_byopenid(openid, name):
    ...
    if name is not None:
        schedules=ConferenceSchedule.query.filter(ConferenceSchedule.name.like('%' + name + '%'),ConferenceSchedule.is_deleted==0).all()
        guest=[]
        schedule_id=[]
        ...
        user_id=[]
        sign_up=ConferenceSignUp.query.filter(ConferenceSignUp.schedule_id.in_(schedule_id)).all()
        for item in sign_up:
